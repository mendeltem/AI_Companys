...
def _ohne_betrachter(url):
    """Aus der Betrachter-Adresse die des Dokuments machen.

    EDGAR verlinkt Berichte mit eingebettetem XBRL oft als
    /ix?doc=/Archives/edgar/data/... - das ist die Anzeigehuelle, ein paar
    Kilobyte Javascript, das den eigentlichen Bericht erst nachlaedt. Wer sie
    speichert, hat den Bericht nicht. Der Pfad im Parameter ist das Dokument.
    """
    treffer = re.search(r"[?&]doc=(/Archives/[^&]+)", url)
    if treffer:
        rest = treffer.group(1)
        return rest if url.startswith("/") else "https://www.sec.gov" + rest
    return url


def _zielpfad(e, belegt):
    """Ablageort eines Dokuments, garantiert eindeutig.

    Zwei Fallen: NVIDIAs Galerie liefert URLs, die auf einen Schraegstrich
    enden und damit keinen Dateinamen tragen; und zwei Einreichungen zum selben
    Quartal koennen gleichnamige Anlagen haben (d142612dex991.htm zweimal am
    selben Tag). Ohne Vorkehrung ueberschreibt die zweite Datei die erste, ohne
    dass irgendwo ein Fehler auftaucht. Bei Gleichstand kommt deshalb ein kurzer
    Abdruck der URL in den Namen - stabil, also beim naechsten Lauf derselbe.
    """
    teile = [t for t in urlparse(e["url"]).path.split("/") if t]
    name = _sicher(teile[-1]) if teile else ""
    if not name or name == "download_pdf":
        name = "dokument"
    ordner = os.path.join(e["ticker"], e.get("quartal_ende") or e.get("eingereicht") or "ohne-datum")
    pfad = os.path.join(ordner, name)
    if belegt.get(pfad, e["url"]) != e["url"]:
        stamm, endung = os.path.splitext(name)
        kurz = hashlib.sha256(e["url"].encode("utf-8")).hexdigest()[:6]
        pfad = os.path.join(ordner, "%s-%s%s" % (stamm, kurz, endung))
    belegt[pfad] = e["url"]
    return pfad
...
def _filing_dokumente(cik, zugang, primaer, form=""):
    ...
    for zeile in re.findall(r"<tr[^>]*>(.*?)</tr>", seite, re.S):
        ...
        if not link or len(felder) < 5:
            continue
        pfad_url = _ohne_betrachter(link.group(1))
...
def reparieren(args):
    """Zielpfade nachziehen, wenn sich die Benennungsregel geaendert hat.

    Aeltere Laeufe legten zwei gleichnamige Anlagen desselben Quartals unter
    denselben Namen ab; die zweite ueberschrieb die erste, ohne Fehler. Dieser
    Durchgang vergibt die Namen neu, streicht die betroffenen Eintraege aus dem
    Ladeprotokoll und raeumt die verwaisten Dateien weg. Danach holt ein
    gewoehnlicher 'laden'-Lauf das Fehlende nach.
    """
    liste = _arbeitsliste_lesen()
    belegt = {}
    alt = {e["url"]: e.get("pfad") for e in liste}

    # Erst die Adressen begradigen: Betrachter-Huellen auf das Dokument
    # zurueckfuehren. Dabei koennen zwei Eintraege auf dieselbe Adresse fallen.
    neu, gesehen, huellen = [], set(), set()
    for e in sorted(liste, key=lambda x: (x["ticker"], x.get("eingereicht") or "", x["url"])):
        echt = _ohne_betrachter(e["url"])
        if echt != e["url"]:
            huellen.add(e["url"])
            e["url"] = echt
        if echt in gesehen:
    ...
    # Zwei getrennte Mengen, sonst zaehlt man dasselbe Dokument doppelt: was neu
    # zu holen ist (nach der neuen Adresse) und was aus dem Ladeprotokoll
    # verschwinden muss (nach der alten).
    nachholen = [e for e in neu if e["url"] not in alt or alt[e["url"]] != e["pfad"]]
    streichen = set(huellen) | {e["url"] for e in nachholen if e["url"] in alt}
    print("%d Dokumente (%d Dubletten entfallen), %d Betrachter-Huellen begradigt, "
          "%d werden neu geholt" % (len(neu), len(liste) - len(neu), len(huellen), len(nachholen)))
    if not nachholen:
        return

    if args.nur_pruefen:
        for e in nachholen[:8]:
            print("   %s\n     -> %s" % (e["url"], e["pfad"]))
        return
    geaendert = streichen

    with open(_pfad("arbeitsliste.jsonl"), "w", encoding="utf-8") as f:
        for e in neu:
            f.write(json.dumps(e, ensure_ascii=False) + "\n")

    # Ladeprotokoll ohne die betroffenen Zeilen neu schreiben, damit 'laden'
    # sie als offen ansieht.
    p = _pfad("geladen.jsonl")
    behalten, raus = [], []
    if os.path.exists(p):
        for z in open(p, encoding="utf-8"):
            try:
                ...
                continue
            (raus if e.get("url") in geaendert else behalten).append(e)
        with open(p, "w", encoding="utf-8") as f:
            for e in behalten:
                f.write(json.dumps(e, ensure_ascii=False) + "\n")

    gebraucht = {e["pfad"] for e in neu}
    entfernt = 0
    for e in raus:
        if e.get("pfad") and e["pfad"] not in gebraucht:
            d = _pfad("dateien", e["pfad"])
            if os.path.exists(d):
                os.remove(d)
                entfernt += 1
    print("%d Eintraege aus dem Ladeprotokoll gestrichen, %d verwaiste Dateien geloescht."
          % (len(raus), entfernt))
    print("Jetzt 'laden' aufrufen - es holt genau diese nach.")
...
def _dateiart(pfad, content_type=""):
    """Womit man es zu tun hat - notfalls am Inhalt erkannt.

    Rund ein Viertel der EDGAR-Anlagen heisst 'ex991' oder 'ex-99_1', ohne
    Punkt und ohne Endung. Nach dem Dateinamen zu gehen hiesse, sie
    stillschweigend zu ueberspringen; deshalb entscheidet hier erst die Endung,
    dann die ersten Bytes, dann der vom Server gemeldete Typ.
    """
    endung = os.path.splitext(pfad)[1].lower()
    if endung in MEDIENENDUNGEN:
        return ""
    try:
        kopf = open(pfad, "rb").read(2048)
    except OSError:
        return ""
    # Der Endung wird nicht blind geglaubt: EDGAR liefert vereinzelt Dateien mit
    # .pdf im Namen und application/pdf im Kopf, die in Wahrheit SGML sind.
    if endung == ".pdf":
        return ".pdf" if kopf[:5] == b"%PDF-" else ".htm"
    if endung in (".htm", ".html", ".txt"):
        return endung
    if kopf[:5] == b"%PDF-":
        return ".pdf"
    klein = kopf.lower()
    if b"<html" in klein or b"<!doctype" in klein or b"<div" in klein or b"<p" in klein:
        return ".htm"
...
def _ein_dokument(e, laenge, ueberlappung, aus):
    ...
    endung = _dateiart(quelle, e.get("content_type", ""))
    if endung == ".pdf":
        t = _pdf_text(quelle)
        if t is None:
            return False, 0, True
    ...
    n = 0
    # Splitter unter etwa einem Satz tragen keine Aussage, verbrauchen aber
    # einen Vektor und tauchen in der Suche als Rauschen auf. Ein einziges
    # kurzes Stueck bleibt trotzdem stehen - sonst faellt ein kurzes Dokument
    # ganz aus dem Index.
    roh = _stuecke(t, laenge, ueberlappung)
    behalten = [s for s in roh if len(s) >= 150] or roh[:1]
    for i, s in enumerate(behalten):
...
def _mit_teilung(texte):
    """Eine Fuhre einbetten; passt sie nicht, halbieren und beide Haelften einzeln.

    llama-server lehnt eine Anfrage ab, deren Token die Batchgroesse
    ueberschreiten - mit 500, ohne zu sagen, wie viel zu viel war. Rekursives
    Halbieren findet die passende Groesse von selbst und kostet nur bei den
    wenigen zu grossen Fuhren einen zweiten Anlauf.
    """
    import numpy as np
    try:
        return _einbetten_roh(texte)
    except Exception:
        if len(texte) == 1:
            raise
        mitte = len(texte) // 2
        return np.vstack([_mit_teilung(texte[:mitte]), _mit_teilung(texte[mitte:])])


# Qwen3-Embedding ist auf eine Asymmetrie trainiert: die Frage bekommt eine
# Anweisungszeile, das Dokument nicht. Gemessen an dieser Sammlung hebt das den
# besten Treffer von 0,62 auf 0,71 und schiebt die richtigen Dokumente nach
# oben. Wer das Modell wechselt, muss diese Zeile pruefen - bei bge-m3 etwa
# waere sie falsch.
FRAGE_ANWEISUNG = ("Instruct: Given a question about a company's quarterly results, "
                   "retrieve the passage from its filing that answers it\nQuery: ")


def _frage_vektor(frage):
    return _einbetten_roh([FRAGE_ANWEISUNG + frage])[0]
...
def einbetten(args):
    ...
    if os.path.exists(index):
        for z in open(index, encoding="utf-8"):
            try:
                ...
                fertig.add(e["id"])
                if e.get("abdruck"):
                    abdruecke.add(e["abdruck"])
            except Exception:
                pass

    # Jedes zehnte Stueck ist wortgleich mit einem anderen: Rechtstext und
    # Bilanzierungsgrundsaetze stehen unveraendert in jedem Quartalsbericht.
    # Wer sie mehrfach einbettet, zahlt dafuer und bekommt eine Trefferliste,
    # in der achtmal derselbe Absatz steht.
    offen, doppelt = [], 0
    for e in _quellzeilen(args):
        if e["id"] in fertig:
            continue
        kern = e["text"].split("\n\n", 1)[-1].strip().lower()
        e["abdruck"] = hashlib.sha256(kern.encode("utf-8")).hexdigest()
        if e["abdruck"] in abdruecke:
            doppelt += 1
            continue
        abdruecke.add(e["abdruck"])
        offen.append(e)
    print("%d Stuecke neu, %d bereits eingebettet, %d wortgleiche uebersprungen"
          % (len(offen), len(fertig), doppelt))
    if not offen:
        return
    if args.grenze:
    ...
    with open(vek, "ab") as fv, open(index, "a", encoding="utf-8") as fi:
        for i in range(0, len(offen), args.fuhre):
            ...
            try:
                # Passt eine Fuhre nicht in den Batch des Servers, antwortet er
                # mit 500. Statt den Lauf zu beenden, wird geteilt und neu
                # versucht - bis hinunter zu einem einzelnen Stueck. So haengt
                # der Durchsatz nicht davon ab, dass --fuhre zufaellig zur
                # Servereinstellung passt.
                m = _mit_teilung([e["text"] for e in teil])
            except Exception as f:
            ...
            for e in teil:
                    ...
                    "quellart": e["quellart"], "abdruck": e.get("abdruck"),
                    "text": e["text"][:400],
                }, ensure_ascii=False) + "\n")
            fv.flush()
            fi.flush()
    ...
    # Ein Lauf, der gleich an der ersten Fuhre scheitert, soll keine leere
    # Vektordatei hinterlassen - sonst sieht 'suchen' einen Bestand, der keiner ist.
    if not n and os.path.exists(vek) and not os.path.getsize(vek):
        os.remove(vek)
...
def suchen(args):
    import numpy as np
    vek, index, meta = _vektoren_pfade()
    # Auf beides pruefen: ein abgebrochener erster Lauf hinterlaesst die
    # Vektordatei leer und ohne Beschreibung daneben.
    if not (os.path.exists(vek) and os.path.exists(meta) and os.path.getsize(vek)):
        sys.exit("Keine Vektoren. Erst 'einbetten' laufen lassen - und dafuer muss der "
                 "Einbettungsserver laufen:\n"
                 "  powershell -ExecutionPolicy Bypass -File werkzeuge\\start-embed.ps1")
    ...
    frage = _frage_vektor(args.frage)
    aehnlich = M @ frage
    ...
    for i in np.argsort(-aehnlich):
        ...
        if erlaubt and (z.get("ticker") or "").upper() not in erlaubt:
            continue
        # Drei Viertel des Bestands sind Berichtsrumpf: Risikohinweise,
        # Bilanzierungsgrundsaetze, Rechtstext. Wer nach dem sucht, was eine
        # Firma zum Quartal gesagt hat, will die Mitteilung, nicht den Anhang.
        if args.art and args.art.lower() not in (z.get("art") or "").lower():
            continue
...
def _aehnliche_stellen(text, sym, vor_datum, anzahl=3):
    ...
    try:
        ...
        n = min(len(zeilen), M.shape[0])
        M, zeilen = M[:n], zeilen[:n]
        werte = M @ _frage_vektor(text[:1500])
        raus = []
...
def taeglich(args):
    ...
    stand = _stand_lesen()
    bekannt = set()
    # Welche Ablageorte schon vergeben sind, damit ein neues Dokument keines
    # der vorhandenen ueberschreibt.
    belegte_pfade = {}
    ...
    for sym, e in F.items():
        ...
        for z in passend:
            ...
            for d in docs:
                ...
                eintr["pfad"] = _zielpfad(eintr, belegte_pfade)
                neu.append(eintr)
        if passend:
...
def main():
    ...
    rp = u.add_parser("reparieren", help="Zielpfade nachziehen und Verwaistes wegraeumen")
    rp.add_argument("--nur-pruefen", action="store_true", help="nur zeigen, was sich aendert")
    rp.set_defaults(f=reparieren)
    ...
    su.add_argument("--art", default="", help="nur diese Dokumentart, z. B. EX-99 oder 10-Q")
